...
auto Trie::Put(std::string_view key, T value) const -> Trie {
	...
	{
		if(len == 0)
			new_root = std::make_shared<TrieNodeWithValue<T>>(std::map<char,std::shared_ptr<const TrieNode>>(),std::make_shared<T>(std::move(value)));
		else
	...
	for(size_t ind = 0;ind < len;ind++)
	{
		...
		{
			if(ind == len-1)
				new_node = std::make_shared<TrieNodeWithValue<T>>(std::map<char,std::shared_ptr<const TrieNode>>(),std::make_shared<T>(std::move(value)));
			else
		...
		{
			if(ind == len-1)
				new_node = std::make_shared<TrieNodeWithValue<T>>(cur_node->children_[key[ind]]->children_,std::make_shared<T>(std::move(value)));
			else
				new_node = std::move(cur_node->children_[key[ind]]->Clone());
...
auto Trie::RemoveHelper(std::string_view key,size_t ind , std::shared_ptr<TrieNode> node) const -> bool
{
	if(node == nullptr)
		return true;
	if(ind == key.size())
	...
	std::shared_ptr<TrieNode> next_child = std::move(node->children_[key[ind]]->Clone());
...
auto Trie::Remove(std::string_view key) const -> Trie {
	...
	std::shared_ptr<TrieNode> new_root = std::move(root->Clone());
